...
function Teamcity(runner, options) {
	...
	let flowId, useStdError, recordHookFailures, actualVsExpected, ignoreHookWithName, displayIgnoredAsIgnored, failTestOnHookError;
	...
	(reporterOptions.failTestOnHookError) ? failTestOnHookError = reporterOptions.failTestOnHookError : failTestOnHookError = process.env['FAIL_TEST_ON_HOOK_ERROR'];
	...
	const findTestTitle = (title) => /^"(before|after) (all|each)" hook for "(.+)"$/.exec(title)[3];
	...
	runner.on(EVENT_TEST_FAIL, function (test, err) {
		...
		const testFlowId = (isHook && !failTestOnHookError) ? hookFlowId : flowId;
		if(actualVsExpected && (err.actual && err.expected)){
			...
			if (useStdError) {
				const title = isHook && failTestOnHookError ? findTestTitle(test.title) : test.title;
				logError(formatString(TEST_FAILED, title, err.message, err.stack, testFlowId));
			} else {
				const title = isHook && failTestOnHookError ? findTestTitle(test.title) : test.title;
				log(formatString(TEST_FAILED, title, err.message, err.stack, testFlowId));
			}
		}
